...
|--------|--------------|
| `lib/news-sources.ts` | Defines RSS feed catalog with categories and fallback URLs |
| `lib/rss-parser.ts` | Fetches RSS/Atom feeds with retry logic and fallback URL support; parses XML and maps to `ParsedNewsItem[]` |
...
| `lib/feed-store.ts` | Client-side storage for user-configurable feed sources (enable/disable, add custom feeds) |
...
| Module | Responsibility |
|--------|--------------|
| `app/api/news/route.ts` | Aggregates all sources, applies server-side filtering (time, category, search query, language) |
| `app/api/chat/route.ts` | Proxies streaming chat requests to various AI providers (OpenAI, Claude, GitHub Models, Ollama, LM Studio, Custom) |
...
```
                    ...
                    ┌─────────────────────────────────────────────┐
                    │              UI Components                       │
                    │  page.js  │  ChatSidebar  │  Settings Panels    │
                    └─────────────────────────────────────────────┘
```
...
│   └── utils.ts                    # Shared utilities (if any)
├── package.json                    # Next.js 16, React 19, Tailwind v4
├── tsconfig.json                   # Path alias @/* → ./*
├── next.config.js                  # Next.js configuration
├── tailwind.config.js              # (Tailwind v4 may not use this)
